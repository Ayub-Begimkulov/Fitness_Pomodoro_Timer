<template>
  <div class="absolute w-screen h-screen" :class="isWorking ? 'gradient-red' : 'gradient-blue'">
    ...
    <div class="flex justify-center items-center h-3/10">
      <button
        v-show="!interval"
        class="bg-transparent flex justify-center items-center text-white border border-white rounded-full w-18 h-18"
        @click="startTimer"
      >
        <span
          v-if="(minutesLeft === focusTime || minutesLeft === breakTime) && secondsLeft === 0"
        >Start</span>
    ...
    setRandomExercises() {
      ...
      while (randomExercises.length < 3) {
        let e = this.allExercises[
          this.getRandomInt(0, this.allExercises.length - 1)
        ];
